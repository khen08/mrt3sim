...
class EventHandler:
    ...
    def _handle_service_period_change(self, event):
        ...
        # Reset insertion timing when starting a new service period
        self.simulation.last_insertion_time = None
    ...
    def _handle_arrival(self, event):
        ...
        # Check currently occupied platform
        if station.platforms[train.direction] is not None:
            platform_conflict = True
            # Find when the occupying train will depart
            departure_events = [e for _, e in self.simulation.event_queue.queue 
                                if e.event_type == "train_departure" and 
                                e.station == station and
                                e.train == station.platforms[train.direction]]
            
            if departure_events:
                earliest_departure = min(departure_events, key=lambda e: e.time)
                required_wait_time = (earliest_departure.time - arrival_time).total_seconds()
        
        # Check for trains in turnaround that will use this platform
        turnaround_events = [e for _, e in self.simulation.event_queue.queue 
                            if e.event_type == "turnaround" and
                            e.station == station and
                            e.train != train]
        
        for turnaround_event in turnaround_events:
            # Calculate when this train will finish turnaround and occupy platform
            turnaround_completion = turnaround_event.time + timedelta(seconds=self.simulation.turnaround_time)
            if turnaround_completion > arrival_time:
                platform_conflict = True
                wait_needed = (turnaround_completion - arrival_time).total_seconds()
                required_wait_time = max(required_wait_time, wait_needed)
        
        # If there's a platform conflict, reschedule the arrival
        if platform_conflict:
            # Add buffer time
            buffer_time = 30  # 30 second buffer
            reschedule_time = arrival_time + timedelta(seconds=required_wait_time + buffer_time)
            
            # Reschedule the arrival event
            self.simulation.schedule_event(
                Event(
    ...
    def _handle_insertion(self, event):
        ...
        # Enforce rolling headway: delay if too close to last insertion
        last = self.simulation.last_insertion_time
        if last is not None:
            earliest = last + timedelta(minutes=self.simulation.active_headway)
            if event.time < earliest:
                # Reschedule this insertion to respect headway
                self.simulation.schedule_event(
                    Event(time=earliest, event_type="train_insertion", train=train, segment=segment)
                )
                return
        # Record this insertion for headway enforcement
        self.simulation.last_insertion_time = event.time
    ...
    def _handle_turnaround(self, event):
        ...
        #======= BOARD/ALIGHT PASSENGERS =======#
        alighted, boarded = station.process_passenger_exchange(
            scheme_map=self.simulation.station_type_map,
            train=train, 
            train_arrival_time=train.arrival_time, 
            train_departure_time=departure_time
        )
        ...
        if event.time <= self.simulation.end_time:
            # Project turnaround finish and only delay if conflicts overlap that projected time
            min_t = self.simulation.turnaround_time
            start_t = departure_time
            projected_finish = start_t + timedelta(seconds=min_t)
            final_finish = projected_finish
            buffer_t = 30  # seconds

            # 1. Existing platform occupant: ensure finish after it frees
            current_occupant = station.platforms.get(train.direction)
            if current_occupant:
                departure_events = [
                    e for _, e in self.simulation.event_queue.queue
                    if e.event_type == "train_departure" and e.station == station and e.train == current_occupant
                ]
                if departure_events:
                    free_time = min(e.time for e in departure_events)
                    if free_time > final_finish:
                        final_finish = free_time + timedelta(seconds=buffer_t)

            # 2. Other turnarounds: avoid overlapping when they emerge
            turnaround_events = [
                e for _, e in self.simulation.event_queue.queue
                if e.event_type == "turnaround" and e.station == station and e.train != train
            ]
            for ev in turnaround_events:
                completion = ev.time + timedelta(seconds=self.simulation.turnaround_time)
                if completion < final_finish:
                    final_finish = max(final_finish, completion + timedelta(seconds=buffer_t))

            # 3. Incoming arrivals: avoid arriving into occupied platform
            arrival_events = [
                e for _, e in self.simulation.event_queue.queue
                if e.event_type == "segment_exit" and e.station == station and e.train != train
            ]
            for ev in arrival_events:
                if ev.time < final_finish:
                    final_finish = max(final_finish, ev.time + timedelta(seconds=buffer_t))

            # Compute actual turnaround duration
            final_turnaround_time = int((final_finish - start_t).total_seconds())
            train.arrival_time = start_t + timedelta(seconds=final_turnaround_time)
            train.current_journey_travel_time = final_turnaround_time
            next_departure_time = train.arrival_time + timedelta(seconds=self.simulation.dwell_time)

            # Schedule the departure after adjusted turnaround
            self.simulation.schedule_event(
                Event(
